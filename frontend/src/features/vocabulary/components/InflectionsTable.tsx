...
export function InflectionsTable({ inflections, partOfSpeech }: InflectionsTableProps) {
  if (inflections.length === 0) {
    const inflectingTypes: Set<string> = new Set(['NOUN', 'VERB', 'ADJECTIVE', 'PRONOUN', 'NUMERAL']);
    const shouldInflect = partOfSpeech && inflectingTypes.has(partOfSpeech);

    if (shouldInflect) {
      return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <svg
            className="inline-block w-12 h-12 text-red-400 mb-3"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
          ...
          <p className="text-red-700 font-medium">Missing inflections</p>
          <p className="text-red-600 text-sm mt-1">
            This {partOfSpeech.toLowerCase()} should have inflections but none are stored.
          </p>
        </div>
      );
    }
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
        <p className="text-gray-500 text-sm">This word does not inflect.</p>
      </div>
    );
  }
